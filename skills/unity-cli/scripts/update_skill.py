...
def _manifest_path(path: Path, skill_root: Path) -> str:
    resolved = path.resolve()
    return (
        str(resolved.relative_to(skill_root)) if resolved.is_relative_to(skill_root) else str(path)
    )


def _manifest(
    snapshot_paths: dict[str, Path], tree: dict[str, Any], skill_root: Path
) -> dict[str, Any]:
    snapshots = {
        ...
        "snapshots": [
            {
                ...
                "source": "live_help",
                "path": _manifest_path(path, skill_root),
                "sha256": _hash(path),
            }
...
def update(
    ...
    *,
    ...
    skill_root = (root / "skills" / "unity-cli").resolve()
    ...
    old_path = skill_root / "data" / "command-tree.json"
    ...
    with tempfile.TemporaryDirectory() as directory:
        ...
        (skill_root / "data").mkdir(parents=True, exist_ok=True)
        shutil.copy2(stage_tree, old_path)
        generate(merged, skill_root / "references")
    (skill_root / "data" / "source-manifest.json").write_text(
        json.dumps(_manifest(snapshot_paths, merged, skill_root), indent=2, ensure_ascii=False)
        + "\n",
        encoding="utf-8",
    )
...
def main() -> int:
    ...
    if args.collect:
        ...
        path = root / "skills" / "unity-cli" / "snapshots" / platform_name / "help.json"
